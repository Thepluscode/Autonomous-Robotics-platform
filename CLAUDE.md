...
- **`/gaia-prime` is the auditor surface, not a copy of `/public`**. `frontend/src/pages/GaiaPrime.jsx` is a calm, terminal-flavored landing that publishes the Ed25519 verification key (`/.well-known/keys.json`), per-zone signed-observation aggregate roots (`GET /api/zones/{id}/attestation`), and a curl cheatsheet for the verify endpoint (`POST /api/observations/verify`). It deliberately uses the regular Earthy theme, not the dark Gaia HUD on `/public` — the audience here is "verify us" reviewers, not investors. The page only calls public endpoints (`PUBLIC_ROUTES`); don't introduce calls that would force auditors to authenticate. The `provenanceAPI` group in `src/lib/api.js` (`getPublicKey`, `getZoneAttestation`, `listObservations`) is the canonical wrapper.
